...
var reminderHelpers = {
  ...
  setReminders: function(userReminders) {
    var recurrence;
    ...
    function createReminderArray() {
      ...
      for (var i = 0; i<userReminders.length; i++) {
        var newReminder = {
            plant: userReminders[i].plant,
            type: userReminders[i].type,
            dates: []
          }

        displayReminders.push(newReminder);
        
        setReminder(userReminders[i].createdAt, userReminders[i].days, userReminders[i].frequency, i);
      }

      // console.log(displayReminders)
      createSortedReminderArrays();
    }
    console.log(userReminders[0])
    createReminderArray()
